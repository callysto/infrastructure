resource "openstack_blockstorage_volume_v3" "zfs" {
...
locals {
  vol_id_1 = "${length(var.existing_volumes) == 0 ?
    element(concat(openstack_blockstorage_volume_v3.zfs.*.id, tolist([""])), 0) :
    element(concat(var.existing_volumes, tolist([""])), 0)
  }"

  vol_id_2 = "${length(var.existing_volumes) == 0 ?
    element(concat(openstack_blockstorage_volume_v3.zfs.*.id, tolist([""])), 1) :
    element(concat(var.existing_volumes, tolist([""])), 1)
  }"
}
