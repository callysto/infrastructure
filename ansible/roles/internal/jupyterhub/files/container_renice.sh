...
# continer which owns it and (optionally) change the scheduling priority
# for the corresponding cgroup.
#
# We assume the CFS scheduler so the quota is implemented as the
# fraction cpu.cfs_quota_us/cpu.cfs_period_us. On an unmodified machine
# cpu.cfs_period_us=100000 so quota is normally specified as 0 <
# cpu.cfs_quota_us < 100000. cpu.cfs_quota_us / cpu.cfs_period_us =
# 100000/100000 corresponds (roughly) to 1 cpu core.
#
#
#######################################################################
# Arguments:
#  pid [-p]   : The PID of a process running inside a docker container
#  quota [-q] : The new quota to use as cfs_quota_us
#######################################################################
